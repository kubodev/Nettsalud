<script>
	...
	$("input[name='fotos[]']").on("change", function(){
            var ff = $(this);
            var idProducto = $(this).parents('form:first').attr("data-idProducto");
			$('#divFotos'+idProducto).css('opacity','0.3');
			$('#cargando').fadeIn("slower");
            var formData = new FormData($(this).parents('form:first')[0]);
            var ruta2 = "<?php echo $Configuracion['Ruta']; ?>panelAdmin/inc/modulos/js/loads/Producto/EditarImagenesPublicacion.php";
            $.ajax({
                url: ruta2,
                type: "POST",
                data: formData,
                ...
                {
                    $("#divFotos"+idProducto).html(datos);
					$('#divFotos'+idProducto).css('opacity','1');
					$('#cargando').fadeOut("slower");
                }
            });
     
     });
	function elimFoto(elim,idProducto,item)
	 {
		 $('#'+item).fadeOut("slower");
		 $('#cargador_php_form').load('<?php echo $Configuracion['Ruta']; ?><?php echo $Configuracion['CarpetaPanel']; ?>/inc/modulos/js/loads/Producto/elimFotoEditarPublicacion.php', {img:elim,idProducto:idProducto });
	 }
	...
	function eliminarImg(id)
	{
		if(cantidadFotos > 1)
		{
			$('#contenedorImg'+id).load(ruta+'inc/modulos/js/loads/Producto/Producto_eliminarFoto.php', {idProductoFoto:id }, function(){$('#contenedorImg'+id).fadeOut("slower",function(){$('#contenedorImg'+id).remove();});});
			cantidadFotos--;
		} else alert("No puedes borrar la única foto del producto");
	}
	function enviarFotos()
	{
		...
		if($(form)[0].checkValidity())
		{
			cargadorOn();
			 $.ajax({
		            type:"POST",
		            url:'<?php echo $ruta; ?>inc/modulos/js/loads/Producto/Producto_editarFotos.php',
		            processData: false,
					contentType: false,
		            data:formData,
	 ...
	 function enviarFormulario()
	{
		var formData = new FormData(form[0]);
					 $.ajax({
				            ...
				            url:'<?php echo $ruta; ?>inc/modulos/js/loads/Producto/Producto_editar.php',
	...
	if($Configuracion['SistemaModelos'] == 1)
	{
		...
		{
			ultimoModelo = $('input[name="ultimoModelo"]').val();
				ultimaEspecificacion.push("1");
				ultimaEspecificacion[id] = 1;
				if(ultimoModelo > 0)
				{ 
						$('<div class="modelo" style="display:none" modelo="'+id+'" id="modelo'+id+'"></div>').insertAfter("#modelo"+ultimoModelo,function(){}); 
				}
				else $('#modelosseleccionados').html('<div class="modelo" style="display:none" modelo="'+id+'" id="modelo'+id+'"></div>');
		...
		function cargarModelosProducto(idProducto,ultimoModelo)
		{
			ultimaEspecificacion.push("1");		
			$('#modelosseleccionados').load(ruta+'inc/modulos/js/loads/Producto/cargarModelosProducto.php', { idProducto: idProducto}, function(){$('input[name="ultimoModelo"]').val(document.getElementById('totalModelos').value);});
		}
		cargarModelosProducto(<?php echo seguridad_sql($_GET['idProducto']); ?>);
		...
		{
			var nuevaespecif = '<div class="especificacion"><div class="lineaform"><label>SKU:</label><input placeholder="Ej: 55588" name="sku'+id+'"></div><div class="lineaform"><label>Stock:</label><input placeholder="Ej: 10" name="stock'+id+'"></div><div class="lineaform"><label>Precio:</label><input placeholder="12990" name="precio'+id+'"></div></div>';	
		...
		function recopilarModelos()
		{
						...
						var l;
						var aux2 = [];
						for(x=0;x<modelos[0].children.length;x++){
		...
		$('span[name="otromodelo"]').click(function(){
			ultimoModelo = $('input[name="ultimoModelo"]').val();
			var nuevoModelo = parseInt(ultimoModelo)+1;
